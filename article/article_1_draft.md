...
Here, we hope to simulate interventions including social distancing, lockdown, and mass testing to gain a better understanding of how these interventions work. In particular, we focus on the effects of these interventions to the recovery and death rates. In this article, we will first focus on the approach we took to model a simple epidemiological simulation using a technique known as agent-based modelling. Further, we will discuss the tools used, as well as how to explore the model yourselves. A separate article will be dedicated to analyses on the simulation results.


### What is Agent-Based Modelling?
<img align="right" src="pictures/sample_run.gif" width="350" height="350" />

Agent-based modelling (ABM) is a technique that models a complex system by reducing it to the interactions between agents and their environment. Here, agents can possess properties, such as age and gender, and can perform actions based on pre-defined set of conditions. The environment possesses properties, can affect the behavior of agents, and vice versa. Observables, such as population growth, can then be measured to infer relationships between agent behavior and the overall behavior of the system. This makes ABMs powerful in that it can be used to observe and quantify the macroscopic behavior of many complex systems over time just by knowing how individual agents behave. This can be achieved without the need to construct differential equations, which may require pre-existing knowledge of relationships between the observables. 

Currently, there are a number of platforms that provide users with the tools needed to build an ABM simulation. For this project, we'll be using Netlogo. 
...
You can move the sliders and/or select values from the drop-down list in the interface tab to modify hyperparameters to their desired values. Clicking the `Setup` button will initiate and update changes to the model. Clicking the `Go` button will run the simulation. A chart plotting the variables of interest, such as the number of infected and recovered agents, are shown on the left side of the interface. On the right side, a view of the simulation is updated at each tick. 
...
At each tick, here defined to be an hour, agents are allowed to move in random directions. If a `susceptible` agent is near an `infected` agent, the latter has a probability of being infected. This is an agent's susceptibility. Here, susceptibility is an agent-level parameter that is used to model a person's hygiene, where a higher `susceptibility` value corresponds to lower hygienic behavior. On the other hand, if an infected agent has been sick for more than or equal to its `recovery-time`, it has a probability of recovering(`recovery-rate`). However, at each tick, an infected agent also has a probability of dying(`death-rate`). 
...
Every 24 ticks, each agent has a probability of being tested(`mass-testing-intensity`). This allows us to simulate mass testing, where everyone is tested regardless of whether the person is showing symptoms or not. To simulate the delay between a person being tested positve to when the person is quarantined, an additional parameter is added, named `quarantine-delay`, which ranges from one day to up to one week. An agent that is quarantined is transferred to an isolated area in order to prevent the agent from infecting other agents. 

### Social Distancing
To quantify the effects of distancing an agent from other people, the parameter named `social-distancing-intensity` is added. Each agent has a probability of maintaining a fixed distance from the agent nearest to it equal to the value of this paramter. This allows us to simulate the behavior of the entire system for cases where everyone complies with social distancing and where only a portion of people do. 
